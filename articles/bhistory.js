...
Kcharts.Chart = Ractive.extend({
	...
	dashBoard: function(){
		...
		var lfactor = options.data.dashBoard.legend.lfactor;
		var pfactor = options.data.dashBoard.legend.pfactor;
	...
	barChart: function(){
		this.setup('barChart')
		var options = this.options;
		var cdata = options.data.barChart.cdata;
		...
		var listener = this.on({
			...
			pieDonut: function(event) {
				...
				listener.cancel();
				history.pushState({chartId: 'pieDonut'}, 'piedonut', 'pieDonutk.html');
				self.pieDonut()
			},
	...
	pieDonut: function(){
		...
		var cdata = options.data.pieDonut.cdata.values;
		var legend = options.data.pieDonut.legend;
		var divId = options.divId;
		var svgWidth = this.nodes[divId].clientWidth;
		var svgHeight = this.nodes[divId].clientHeight;	
